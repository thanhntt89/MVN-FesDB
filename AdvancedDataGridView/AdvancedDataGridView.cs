    ...
    public class AdvancedDataGridView : DataGridView
    {
        ...
        {    
            ...
            if (sortEventArgs.Cancel == false)
            {
                ...
                if (datasource != null)
                {
                    int currentHorizontalScrollingOffset = this.HorizontalScrollingOffset;
                    datasource.Sort = sortEventArgs.SortString;
                    this.HorizontalScrollingOffset = currentHorizontalScrollingOffset;
                }
            ...
            if (valueToFind != null && RowCount > 0 && ColumnCount > 0 && (columnName == null || (Columns.Contains(columnName) && Columns[columnName].Visible)))
            {
                ...
                if (columnName != null)
                {
                    ...
                    for (int r = rowIndex; r < RowCount; r++)
                    {
                        ...
                        try
                        {
                            value = Rows[r].Cells[c].FormattedValue.ToString();
                        }
                        catch
                        {
                        }
        ...
        {
            if (this.AllowUserToAddRows && (this.CurrentRow != null && this.CurrentRow.Index == this.RowCount - 1))
            {               
                return;
            }
            if (Columns.Contains(e.Column))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (bs != null)
                        {
                            filterMenu.Show(this, rect.Left, rect.Bottom, this, column.DataPropertyName);
                        }
                        //else
                        //    filterMenu.Show(this, rect.Left, rect.Bottom, this, column.DataPropertyName, MenuStrip.GetValuesForFilter(this, column.Name));
                    }
                }
            }
        }
