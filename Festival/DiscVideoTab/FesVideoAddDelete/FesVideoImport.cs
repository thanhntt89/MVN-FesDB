...
{
    public partial class FesVideoImport : FormBase
    {
        private FesVideoAddDeleteBusiness fesVideoAddDeleteBusiness;
        private DataTable dtImport = new DataTable();
        private int totalRecords = 0;

        public FesVideoImport()
        {
            InitializeComponent();
        }

        private void FesVideoImport_Load(object sender, EventArgs e)
        {
            LoadInit();
        }
        ...
        {
            if (!string.IsNullOrWhiteSpace(Properties.Settings.Default.FES_VIDEO_MANAGEMENT_背景ファイルサイズ取込))
                txtInputFilePath.Text = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + "\\" + Properties.Settings.Default.FES_VIDEO_MANAGEMENT_背景ファイルサイズ取込;
        }
        ...
        private bool Valid()
        {
            ...
            {
                dtImport.Columns.Add("背景ファイル名", typeof(string));
            ...
            for (int index = 0; index < totalRecords; index++)
            {
                ...
                {
                    this.ClosedWaiting();
                    MessageBox.Show(string.Format("2カラム以外の行があったため取り込めませんでした。\n{0} 行目 背景ファイル名", index+1), GetResources.GetResourceMesssage(Constants.ERROR_TITLE_MESSAGE), MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                ...
                {
                    this.ClosedWaiting();
                    MessageBox.Show(string.Format("{0}行目 Fes_データサイズのフォーマットが違います。", index+1), GetResources.GetResourceMesssage(Constants.ERROR_TITLE_MESSAGE), MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }                
        ...
        private void ExecuteImport()
        {
            try
            {
                ...
                fesVideoAddDeleteBusiness.ImportVideo(dtImport, ref sucessCount, ref failsCount, ref errorMessage);
